    ...
    try:
        ...
        for test_type in test_types:
            ...
            filtered_data[test_type] = [
                item[1] / 1_000_000 for item in data if item[0] == test_type
            ]
...
def calculate_speedup(sqlite_data, columnar_data, operation_name, mode_name):
    ...
    if len(sqlite_no_outliers) > 0 and len(columnar_no_outliers) > 0:
        speedup_no_outliers = np.mean(sqlite_no_outliers) / np.mean(
            columnar_no_outliers
        )
...
def analyze_mode_data(mode_data, mode_name):
    ...
    if len(mode_data["test_sqlite"]) > 0 and len(mode_data["test_columnar"]) > 0:
        speedup = calculate_speedup(
            mode_data["test_sqlite"], mode_data["test_columnar"], "Insert", mode_name
        )
        if speedup:
            ...
            if not np.isnan(speedup["speedup_no_outliers"]):
                print(
                    f"  Speedup (no outliers): {speedup['speedup_no_outliers']:.2f}x faster"
                )
    ...
    if len(mode_data["avg_sqlite"]) > 0 and len(mode_data["avg_columnar"]) > 0:
        speedup = calculate_speedup(
            mode_data["avg_sqlite"], mode_data["avg_columnar"], "Average", mode_name
        )
        if speedup:
            speedup_results.append(speedup)
    ...
    if len(mode_data["test_sqlite"]) > 0 and len(mode_data["test_columnar"]) > 0:
        t_result = perform_t_test(
            mode_data["test_sqlite"],
            mode_data["test_columnar"],
            f"{mode_name} Insert SQLite",
            f"{mode_name} Insert Columnar",
        )
    ...
    if len(mode_data["avg_sqlite"]) > 0 and len(mode_data["avg_columnar"]) > 0:
        t_result = perform_t_test(
            mode_data["avg_sqlite"],
            mode_data["avg_columnar"],
            f"{mode_name} Average SQLite",
            f"{mode_name} Average Columnar",
        )
...
def cross_mode_comparisons(sync_data, concurrent_data):
    ...
    for test_type in test_types:
        if len(sync_data[test_type]) > 0 and len(concurrent_data[test_type]) > 0:
            t_result = perform_t_test(
                sync_data[test_type],
                concurrent_data[test_type],
                f"Sync {test_type}",
                f"Concurrent {test_type}",
            )
...
def analyze_files():
    ...
    concurrent_results, concurrent_speedup = analyze_mode_data(
        concurrent_data, "concurrent"
    )
    ...
    if all_speedups:
        ...
        for speedup in all_speedups:
            ...
            if speedup["speedup_mean"] > 1:
                ...
                print(
                    f"  ✗ SQLite actually performs better (Columnar is {1 / speedup['speedup_mean']:.2f}x slower)"
                )
            else:
